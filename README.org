...
As an example, let's compare the same simple model run but tracking it with two
...
Then, we create a simple logistic regression model and train that model on the
iris dataset, increasing the number of iterations and changing the
...
- [ ] add description to pypi release
- [ ] add documentation through rtd or github pages
